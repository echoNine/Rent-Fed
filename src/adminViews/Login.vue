    ...
    <div class="login-container">
      ...
      <div class="login-box" v-show="isLogin">
        ...
        <el-form :model="LoginForm" :rules="LoginFormRules" ref="LoginFormRef" class="login-form">
          ...
          <p style="color: #87abc4;font-size: 14px">测试账户: 2208839499@qq.com 密码: 123123</p>
        </el-form>
      </div>
  ...
  methods: {
    ...
    login () {
      this.$refs.LoginFormRef.validate(valid => {
        if (valid) {
          this.$ajax.post('/backend/admin/login', {
    ...
    submitInfo () {
      ...
      this.$ajax.post('/backend/admin/perfectInfo', {
    ...
    register () {
      this.$refs.RegisterFormRef.validate(valid => {
        if (valid) {
          this.$ajax.post('/backend/admin/register', {
    ...
    send () {
      this.$refs.RegisterFormRef.validate((valid) => {
        if (valid) {
          this.$ajax.post('/backend/admin/send', {
            email: this.RegisterForm.email,
            pwd: this.RegisterForm.pwd
          }).then(res => {
            if (res.data.success) {
